...
$searchSql = $search ? "WHERE item_code LIKE '%$search%' OR item_name LIKE '%$search%'" : '';

// Count total rows for pagination
$countResult = $conn->query("SELECT COUNT(*) as total FROM item $searchSql");
$totalRows = $countResult->fetch_assoc()['total'];
$totalPages = ceil($totalRows / $limit);

// Fetch paginated results
$result = $conn->query("
    SELECT i.*, c.category, sc.sub_category
    FROM item i
...
<div class="container mt-5">
    ...
    <?php if (isset($_GET['success'])): ?>
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            <?= $_GET['success'] === '1' ? 'Item added!' : ($_GET['success'] === 'updated' ? 'Item updated!' : 'Item deleted!') ?>
